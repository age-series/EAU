...
public class GhostGroup {
    ...
    public void addElement(int x, int y, int z) {
        elementList.add(new GhostGroupElement(x, y, z, EAU.ghostBlock, GhostBlock.tCube));
    }
    ...
    public boolean canBePloted(World world, int x, int y, int z) {
        for (GhostGroupElement element : elementList) {
            if (false == EAU.ghostManager.canCreateGhostAt(world, x + element.x, y + element.y, z + element.z))
                return false;
        }
        return true;
    }
    ...
    public boolean plot(Coordonate coordonate, Coordonate observerCoordonate, int UUID) {
        ...
        for (GhostGroupElement element : elementList) {
            ...
            EAU.ghostManager.createGhost(offsetCoordonate, observerCoordonate, UUID, element.block, element.meta);
        }
        return true;
    }

    public void erase(Coordonate observerCoordonate) {
        EAU.ghostManager.removeGhostAndBlockWithObserver(observerCoordonate);
    }

    public void erase(Coordonate observerCoordonate, int uuid) {
        EAU.ghostManager.removeGhostAndBlockWithObserver(observerCoordonate, uuid);
    }

    public void eraseGeo(Coordonate coordonate) {
        for (GhostGroupElement element : elementList) {
            EAU.ghostManager.removeGhostAndBlock(coordonate.newWithOffset(element.x, element.y, element.z));
        }
    }
