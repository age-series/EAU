...
public class AutoMinerSlowProcess implements IProcess, INBTTReady {
    ...
    private final ArrayList<ItemStack> itemsToDrop = new ArrayList<ItemStack>(4);
            ...
            if (energyCounter >= energyTarget) {
                ...
                switch (job) {
                    ...
                    case pipeAdd:
                        ...
                        EAU.ghostManager.createGhost(jobCoord, miner.node.coordonate, jobCoord.y);
                    ...
                    case pipeRemove:
                        ...
                        EAU.ghostManager.removeGhostAndBlock(jobCoord);
                        if (miner.getInventory().getStackInSlot(AutoMinerContainer.MiningPipeSlotId) == null) {
                            miner.getInventory().setInventorySlotContents(AutoMinerContainer.MiningPipeSlotId, EAU.miningPipeDescriptor.newItemStack(1));
                        } else {
                            miner.getInventory().decrStackSize(AutoMinerContainer.MiningPipeSlotId, -1);
                        }
    ...
    private void setupJob() {
        ...
        int scannerRadius = EAU.autominerRange;
    ...
    private boolean checkIsOre(Coordonate coordonate) {
        ...
        return PortableOreScannerItem.RenderStorage.getBlockKeyFactor()[Block.getIdFromBlock(block) +
    ...
    private void destroyPipe() {
        dropPipe();
        EAU.ghostManager.removeGhostAndBlockWithObserverAndNotUuid(miner.node.coordonate, miner.descriptor.getGhostGroupUuid());
        pipeLength = 0;
        miner.needPublish();
    }
    ...
    private void dropPipe() {
        ...
        for (coord.y = miner.node.coordonate.y - 1; coord.y >= miner.node.coordonate.y - pipeLength; coord.y--) {
            Utils.dropItem(EAU.miningPipeDescriptor.newItemStack(1), coord);
        }
    }
