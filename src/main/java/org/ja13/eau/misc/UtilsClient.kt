...
object UtilsClient {
    ...
    private var lightMapTexUnitTextureEnable = false
    ...
    fun distanceFromClientPlayer(xCoord: Int, yCoord: Int, zCoord: Int): Float {
    ...
    @JvmStatic
    fun distanceFromClientPlayer(tileEntity: org.ja13.eau.node.six.SixNodeEntity): Float {
        return distanceFromClientPlayer(tileEntity.xCoord, tileEntity.yCoord, tileEntity.zCoord)
    }
    ...
    @JvmStatic
    fun drawHaloNoLightSetup(halo: Obj3DPart?) {
        if (halo == null) return
    ...
    @Suppress("unused")
    @JvmStatic
    fun drawHalo(halo: Obj3DPart?) {
        disableLight()
        enableBlend()
        drawHaloNoLightSetup(halo)
        enableLight()
        disableBlend()
    }
    ...
    fun drawEnergyBare(type: ItemRenderType, e: Float) {
        drawIcon(type, ResourceLocation("eau", "sprites/portablebatteryoverlay.png"))
        val x = 13f
        val y = 14f - e * 12f
        GL11.glDisable(GL11.GL_TEXTURE_2D)
    ...
    fun drawConnectionPinSixNode(d: Float, w: Float, h: Float) {
        ...
        disableTexture()
        GL11.glBegin(GL11.GL_QUADS)
        GL11.glVertex3f(-w2, d1, 0f)
        GL11.glVertex3f(w2, d1, 0f)
        GL11.glVertex3f(w2, d1, h1)
        GL11.glVertex3f(-w2, d1, h1)
        GL11.glEnd()
        enableTexture()
    }
    ...
    @JvmStatic
    fun drawItemStack(par1ItemStack: ItemStack?, x: Int, y: Int, gui: Boolean) {
    ...
    @JvmStatic
    fun clientDistanceTo(t: org.ja13.eau.node.transparent.TransparentNodeEntity?): Double {
    ...
    fun sendPacketToServer(bos: ByteArrayOutputStream) {
        val packet = C17PacketCustomPayload(org.ja13.eau.EAU.channelName, bos.toByteArray())
        org.ja13.eau.EAU.eventChannel.sendToServer(FMLProxyPacket(packet))
    ...
    @JvmStatic
    fun showItemTooltip(src: List<String>, dst: MutableList<Any?>?) {
        if (src.isEmpty()) return
        val localSrc = src.filter{!it.isBlank()}
        @Suppress("UNCHECKED_CAST") // Shhhhh, Minecraft's type system is sleeping.
        val localDst = (dst as MutableList<String>)
        if (showItemTooltip() || localSrc.size <= 1) {
            localDst.addAll(localSrc)
        } else {
            localDst.add("\u00a7F\u00a7o<" + org.ja13.eau.i18n.I18N.tr("Hold shift") + ">")
        }
    }

    private fun showItemTooltip(): Boolean {
