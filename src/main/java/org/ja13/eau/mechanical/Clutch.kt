...
import org.ja13.eau.sim.process.destruct.DelayedDestruction
...
class ClutchPlateItem(
    ...
    val wearSpeed: Float, val explodes: Boolean
...
class ClutchPinItem(name: String) : org.ja13.eau.generic.GenericItemUsingDamageDescriptorWithComment(name, tr("Prevents clutches from slipping\nagain after they stop.").split("\n").toTypedArray())

class ClutchDescriptor(name: String, override val obj: org.ja13.eau.misc.Obj3D) : SimpleShaftDescriptor(name, ClutchElement::class, ClutchRender::class, org.ja13.eau.node.transparent.EntityMetaTag.Basic) {
    companion object {
    ...
    override val static: Array<out org.ja13.eau.misc.Obj3D.Obj3DPart> = arrayOf(
        obj.getPart("Stand"),
        obj.getPart("Cowl")
    )
    override val rotating: Array<out org.ja13.eau.misc.Obj3D.Obj3DPart> = emptyArray()
...
class ClutchElement(node: org.ja13.eau.node.transparent.TransparentNode, desc_: org.ja13.eau.node.transparent.TransparentNodeDescriptor) : SimpleShaftElement(node, desc_) {
    ...
    val inv = org.ja13.eau.node.transparent.TransparentNodeElementInventory(2, 1, this)
    override fun getInventory() = inv
    override fun newContainer(side: Direction?, player: EntityPlayer?) = ClutchContainer(player, inv)
    override fun hasGui() = true

    val inputGate = org.ja13.eau.sim.nbt.NbtElectricalGateInput("clutchIn")
    var slipping = true
    override fun getElectricalLoad(side: Direction?, lrdu: LRDU?): org.ja13.eau.sim.ElectricalLoad? = inputGate
    override fun getConnectionMask(side: Direction?, lrdu: LRDU?): Int = org.ja13.eau.node.NodeBase.MASK_ELECTRIC
    ...
    val clutchPlateDescriptor: ClutchPlateItem?
        get() {
            ...
            return (stack.item!! as org.ja13.eau.generic.GenericItemUsingDamage<GenericItemUsingDamageDescriptor>).getDescriptor(stack) as ClutchPlateItem
        }
    ...
    inner class ClutchPreProcess : org.ja13.eau.sim.IProcess {
    ...
    inner class ClutchPostProcess : org.ja13.eau.sim.IProcess {
        override fun process(time: Double) {
            ...
            */
            if(slipping) {
                if(plateDescriptor.explodes && (faster.rads - slower.rads) > 5) {
                    DelayedDestruction(
                        org.ja13.eau.sim.process.destruct.WorldExplosion(this@ClutchElement).machineExplosion(),
                        0.0
                    )
                    return
                }
                ...
                if(Math.abs(flow) > maxE && !hasPin) {
                    ...
                    leftShaft.rads = Math.sqrt(2 * leftEnergy / (mass * org.ja13.eau.EAU.shaftEnergyFactor))
                    rightShaft.rads = Math.sqrt(2 * rightEnergy / (mass * org.ja13.eau.EAU.shaftEnergyFactor))
                }
            }
        }
    }
    ...
    */
    ...
    override fun getWaila(): MutableMap<String, String> {
        ...
        info.put("Clutching", Utils.plotVolt(inputGate.voltage))
        if(org.ja13.eau.EAU.wailaEasyMode) {
...
class ClutchRender(entity: org.ja13.eau.node.transparent.TransparentNodeEntity, desc_: org.ja13.eau.node.transparent.TransparentNodeDescriptor) : ShaftRender(entity, desc_) {
    ...
    override fun draw() {
        ...
        preserveMatrix {
            ...
            for (lrdu in LRDU.values()) {
                ...
                org.ja13.eau.cable.CableRender.drawCable(cableRender, mask, connectionType)
            }
        }
    }
    ...
    override fun networkUnserialize(stream: DataInputStream) {
        ...
        if(lastSlipping && !slipping && hasPin) play(org.ja13.eau.sound.SoundCommand(desc.slipStopSound))
...
class ClutchContainer(player: EntityPlayer?, inv: IInventory) : org.ja13.eau.misc.BasicContainer(
    player, inv, arrayOf(
        org.ja13.eau.generic.GenericItemUsingDamageSlot(inv, 0, 176 / 2 - 16 / 2 - 17 + 4, 42 - 16 / 2, 1, ClutchPlateItem::class.java, org.ja13.eau.gui.ISlotSkin.SlotSkin.medium, arrayOf(tr("Clutch Plate"))),
        org.ja13.eau.generic.GenericItemUsingDamageSlot(inv, 1, 176 / 2 - 16 / 2 + 17 + 4, 42 - 16 / 2, 1, ClutchPinItem::class.java, org.ja13.eau.gui.ISlotSkin.SlotSkin.medium, arrayOf(tr("Clutch Pin")))
    )
)

class ClutchGui(player: EntityPlayer?, inv: IInventory, val render: ClutchRender) : org.ja13.eau.gui.GuiContainerEln(ClutchContainer(player, inv)) {
