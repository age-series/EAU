...
import org.ja13.eau.sim.process.destruct.ShaftSpeedWatchdog
...
open class ShaftNetwork() : INBTTReady {
    ...
    var energy: Double
        get() = if (mass.isFinite() && rads.isFinite()) mass * rads * rads * 0.5 * org.ja13.eau.EAU.shaftEnergyFactor else 0.0
        set(value) {
            if(value < 0 || !value.isFinite())
                rads = 0.0
            else
                rads = Math.sqrt(2 * value / ((if(mass.isFinite()) mass else 0.0) * org.ja13.eau.EAU.shaftEnergyFactor))
        }
    ...
    fun mergeShafts(other: ShaftNetwork, invoker: ShaftElement?): ShaftNetwork {
        ...
        if(!loadMerge) {
            ...
            if (wouldExplode(this, other) && invoker != null) {
                ...
                DelayedDestruction(
                    org.ja13.eau.sim.process.destruct.WorldExplosion(invoker.coordonate()).machineExplosion(),
    ...
    private fun getNeighbours(from: ShaftElement): ArrayList<ShaftNeighbour> {
        ...
        for (dir in from.shaftConnectivity) {
            ...
            c.move(dir)
            val to = org.ja13.eau.node.NodeManager.instance!!.getTransparentNodeFromCoordinate(c)
