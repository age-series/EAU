...
public class ServerEventListener {
    ...
    private final HashSet<Integer> loadedWorlds = new HashSet<Integer>();
    ...
    public void onWorldUnload(Unload e) {
        ...
        try {
            ...
            EAU.ghostManager.unload(e.world.provider.dimensionId);
        } catch (Exception ex) {
            ex.printStackTrace();
        }
    ...
    static void readFromEaWorldNBT(NBTTagCompound nbt) {
        ...
        try {
            EAU.ghostManager.loadFromNBT(nbt.getCompoundTag("ghost"));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    static void writeToEaWorldNBT(NBTTagCompound nbt, int dim) {
        ...
        try {
            EAU.ghostManager.saveToNBT(Utils.newNbtTagCompund(nbt, "ghost"), dim);
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private class FileNames {
        ...
        private String getEaWorldSaveName(World w) {
            return Utils.getMapFolder() + "data/EAUWorld" + w.provider.dimensionId + ".dat";
        }
    }
}
