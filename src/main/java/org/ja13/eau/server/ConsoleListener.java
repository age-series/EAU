...
public class ConsoleListener extends CommandBase {
    ...
    private final List<String> cmdVisibleList;
    ...
    public String getCommandUsage(ICommandSender icommandsender) {
        //TODO Rewrite
        String str = Color.COLOR_DARK_CYAN + "EAU mod console." + Color.COLOR_BRIGHT_GREY + " Type \"\\eau \" + TAB";
        return str;
    }
        ...
        switch (argc) {
            case 1:
                ...
                if (astring[0].isEmpty()) {
                    icommandsender.addChatMessage(new ChatComponentText(Color.COLOR_DARK_CYAN + "EAU >"));
                ...
                //icommandsender.addChatMessage(new ChatComponentText(Color.COLOR_DARK_GREY + "EAU > Console > Available commands :\n"));
    ...
    public void processCommand(ICommandSender ics, String[] astring) {
        ...
        if (cmd.isEmpty()) { //Will normally never append.
            ...
            cprint(ics, Color.COLOR_DARK_CYAN + "EAU > " + Color.COLOR_DARK_YELLOW + cmdNameStr_listCmd);
            ...
            cprint(ics, Color.COLOR_DARK_CYAN + "EAU > " + Color.COLOR_DARK_YELLOW + cmdNameStr_about);
            cprint(ics, strOffsetL0 + EAU.NAME + " - Codename " + EAU.MODID.toUpperCase(Locale.ROOT));
            cprint(ics, strOffsetL0 + "V" + Version.MAJOR + '.' + Version.MINOR + " r" + Version.REVISION);
            String authorsStr = "";
            for (int idx = 0; idx < EAU.AUTHORS.length; idx++)
                authorsStr += EAU.AUTHORS[idx] + ' ';
            cprint(ics, strOffsetL0 + "Authors: " + Color.COLOR_DARK_GREY + authorsStr);
            cprint(ics, strOffsetL0 + "Website: " + Color.COLOR_DARK_GREY + EAU.URL);
        } else if (cmd.equalsIgnoreCase(cmdNameStr_aging)) {
            cprint(ics, Color.COLOR_DARK_CYAN + "EAU > " + Color.COLOR_DARK_YELLOW + cmdNameStr_aging);
            if (!checkArgCount(ics, astring, 1))
                return;
            ...
            cprint(ics, Color.COLOR_BRIGHT_YELLOW + "Electrical Age Unleashed Version Info:");
            ...
            cprint(ics, Color.COLOR_DARK_CYAN + "EAU > " + Color.COLOR_DARK_YELLOW + cmdNameStr_lampAging);
            if (!checkArgCount(ics, astring, 1))
                return;
            ConsoleArg<Boolean> arg0 = getArgBool(ics, astring[1]);
            if (!arg0.valid)
            ...
            cprint(ics, Color.COLOR_DARK_CYAN + "EAU > " + Color.COLOR_DARK_YELLOW + cmdNameStr_batteryAging);
            if (!checkArgCount(ics, astring, 1))
                return;
            ConsoleArg<Boolean> arg0 = getArgBool(ics, astring[1]);
            if (!arg0.valid)
            ...
            cprint(ics, Color.COLOR_DARK_CYAN + "EAU > " + Color.COLOR_DARK_YELLOW + cmdNameStr_heatFurnaceFuel);
            if (!checkArgCount(ics, astring, 1))
                return;
            ConsoleArg<Boolean> arg0 = getArgBool(ics, astring[1]);
            if (!arg0.valid)
            ...
            cprint(ics, Color.COLOR_DARK_CYAN + "EAU > " + Color.COLOR_DARK_YELLOW + cmdNameStr_newWind);
            if (!checkArgCount(ics, astring, 0))
                return;
            EAU.wind.newWindTarget();
            cprint(ics, strOffsetL0 + "New random wind amplitude target : " + EAU.wind.getTargetNotFiltered());
        } else if (cmd.equalsIgnoreCase(cmdNameStr_regenOre)) {
            cprint(ics, Color.COLOR_DARK_CYAN + "EAU > " + Color.COLOR_DARK_YELLOW + cmdNameStr_regenOre);
            if (!checkArgCount(ics, astring, 1))
                return;
            ConsoleArg<Boolean> arg0 = getArgBool(ics, astring[1]);
            if (!arg0.valid)
                return;
            EAU.saveConfig.reGenOre = arg0.value;
            ...
            cprint(ics, Color.COLOR_DARK_CYAN + "EAU > " + Color.COLOR_DARK_YELLOW + cmdNameStr_generateLangFileTemplate);
            ...
            cprint(ics, Color.COLOR_DARK_CYAN + "EAU > " + Color.COLOR_DARK_YELLOW + cmdNameStr_killMonstersAroundLamps);
            if (!checkArgCount(ics, astring, 1))
                return;
            ConsoleArg<Boolean> arg0 = getArgBool(ics, astring[1]);
            if (!arg0.valid)
                return;
            EAU.killMonstersAroundLamps = arg0.value;
            ...
            int ssc = EAU.simulator.mna.systems.size();
            int ct = 0;
            for (SubSystem s: EAU.simulator.mna.systems) {
                ct += s.component.size();
                dumpSubSystems += s.toString() + "\n";
            }

            File f = new File("eauDumpSubSystems.txt");
            try {
            ...
            cprint(ics, Color.COLOR_DARK_CYAN + "EAU > " + Color.COLOR_DARK_RED + "Error: Unknown command.");
        }
    }

    private boolean checkArgCount(ICommandSender ics, String[] args, int exceptedArgc) {
        if ((args.length - 1) != exceptedArgc) {
            cprint(ics, Color.COLOR_DARK_CYAN + "EAU > " + Color.COLOR_DARK_YELLOW + args[0]);
    ...
    private void commandMan(ICommandSender ics, String cmd) {
        cprint(ics, (Color.COLOR_DARK_CYAN + "EAU >" + Color.COLOR_DARK_YELLOW + " man > " + Color.COLOR_WHITE + cmd));
        if (cmd.equalsIgnoreCase(cmdNameStr_man)) {
            ...
            cprint(ics, strOffsetL0 + "Lists all EAU publicly available commands.");
            cprint(ics, "");
            cprint(ics, strOffsetL0 + "No input parameters.");
            cprint(ics, "");
        } else if (cmd.equalsIgnoreCase(cmdNameStr_about)) {
            ...
            cprint(ics, strOffsetL0 + "When set, regenerates EAU ores at the next map reload.");
            ...
            cprint(ics, strOffsetL0 + "(See \"EAU.cfg\" for permanent effect.)");
            cprint(ics, "");
            cprint(ics, strOffsetL0 + "Parameters :");
