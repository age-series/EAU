...
public class OreRegenerate {
    ...
    public void tick(ServerTickEvent event) {
        ...
        for (int idx = 0; idx < 1; idx++) {
            if (!jobs.isEmpty()) {
                ...
                if (!EAU.saveConfig.reGenOre && !EAU.forceOreRegen) return;
                ...
                for (int y = 0; y < 60; y += 2) {
                    for (int z = y & 1; z < 16; z += 2) {
                        for (int x = y & 1; x < 16; x += 2) {
                            if (chunk.getBlock(x, y, z) == EAU.oreBlock) {
                ...
                for (OreDescriptor d : EAU.oreItem.descriptors) {
    ...
    public void chunkLoad(ChunkEvent.Load e) {
        ...
        if (e.world.isRemote || (EAU.saveConfig != null && !EAU.saveConfig.reGenOre)) return;
        Chunk c = e.getChunk();
