...
public class ElectricalMathGui extends GuiContainerEln {
    ...
    public void initGui() {
        ...
        expression.setComment(new String[]{I18N.tr("Output voltage formula"),
            I18N.tr("Inputs are") + " \u00a74A \u00a72B \u00a71C"});
    }
    ...
    protected void postDraw(float f, int x, int y) {
        ...
        if (!expression.getText().equals(render.expression)) {
            c = 0xFF404040;
            helper.drawString(8 + 44 / 2, 29, c, I18N.tr("Waiting for completion..."));
        } else if (expression.getText().equals("")) {
            c = 0xFF404040;
            helper.drawString(8 + 44 / 2, 29, c, I18N.tr("Equation required!"));
        } else if (render.equationIsValid) {
            if (redNbr >= render.redstoneRequired)
                c = 0xFF108F00;
            else
                c = 0xFFFF0000;
            helper.drawString(8 + 44 / 2, 29, c, I18N.tr("%1$ Redstone(s) required", render.redstoneRequired));
        } else {
            c = 0xFFFF0000;
            helper.drawString(8 + 44 / 2, 29, c, I18N.tr("Invalid equation!"));
        }
    }
}
