...
public class ElectricalSensorElement extends SixNodeElement implements IConfigurable {
    ...
    public int getConnectionMask(LRDU lrdu) {
        ...
        if (!descriptor.voltageOnly) {
            if (front.left() == lrdu && cable) return NodeBase.MASK_ELECTRIC;
            if (front.right() == lrdu && cable) return NodeBase.MASK_ELECTRIC;
        } else {
            if (front.inverse() == lrdu && cable) return NodeBase.MASK_ELECTRIC;
        }
    ...
    public void initialize() {
        ...
        EAU.applySmallRs(aLoad);
        if (bLoad != null) EAU.applySmallRs(bLoad);
    }
