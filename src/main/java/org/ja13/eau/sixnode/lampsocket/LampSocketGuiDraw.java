...
public class LampSocketGuiDraw extends GuiContainerEln {
    ...
    public void initGui() {
        ...
        channel.setComment(0, I18N.tr("Specify the supply channel"));
    ...
    protected void preDraw(float f, int x, int y) {
        ...
        if (lampRender.grounded)
            buttonGrounded.displayString = I18N.tr("Parallel");
        else
            buttonGrounded.displayString = I18N.tr("Serial");

        if (lampRender.poweredByLampSupply) {
            buttonSupplyType.displayString = I18N.tr("Powered by Lamp Supply");
            channel.setVisible(true);
            if (inventory.getStackInSlot(LampSocketContainer.cableSlotId) == null)
                channel.setComment(1, "§4" + I18N.tr("Cable slot empty"));
            else if (lampRender.isConnectedToLampSupply)
                channel.setComment(1, "§2" + I18N.tr("connected to " + lampRender.channel));
            else
                channel.setComment(1, "§4" + tr("%1$ is not in range!", lampRender.channel));
        } else {
            channel.setVisible(false);
            buttonSupplyType.displayString = I18N.tr("Powered by cable");
        }

        alphaZ.setComment(0, I18N.tr("Orientation: %1$°", (int) alphaZ.getValue()));
    }
}
