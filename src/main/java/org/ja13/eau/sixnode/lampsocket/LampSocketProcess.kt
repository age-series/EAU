...
import org.ja13.eau.sixnode.lampsupply.LampSupplyElement.PowerSupplyChannelHandle
...
class LampSocketProcess(var lamp: org.ja13.eau.sixnode.lampsocket.LampSocketElement) : org.ja13.eau.sim.IProcess, INBTTReady /*,LightBlockObserver*/ {
    @JvmField
    var light = 0 // 0..15
    @JvmField
    var alphaZ = 0.0
    ...
    private fun findBestSupply(here: Coordonate, forceUpdate: Boolean = false): Pair<Double, PowerSupplyChannelHandle>? {
        val chanMap = org.ja13.eau.sixnode.lampsupply.LampSupplyElement.channelMap[lamp.channel] ?: return null
        ...
        val list = org.ja13.eau.sixnode.lampsupply.LampSupplyElement.channelMap[lamp.channel]?.filterNotNull() ?: return null
        val chanHand = list
    ...
    override fun process(time: Double) {
        ...
        val cableStack = lamp.inventory.getStackInSlot(org.ja13.eau.sixnode.lampsocket.LampSocketContainer.cableSlotId)
        // LampDescriptor? is *important* here. Otherwise, NPE.
        val lampDescriptor = (lampStack?.item as? org.ja13.eau.generic.GenericItemUsingDamage<*>)?.getDescriptor(lampStack) as? LampDescriptor
        ...
        if (lampDescriptor != null) {
            val bulbCanAge = !(lampDescriptor.type == LampDescriptor.Type.LED && org.ja13.eau.EAU.ledLampInfiniteLife) && org.ja13.eau.server.SaveConfig.instance.electricalLampAging
    ...
    fun placeSpot(newLight: Int) {
        ...
        if (!exit) {
            ...
            while (newCoord != lamp.sixNode.coordonate) {
                ...
                if (block === Blocks.air || block === org.ja13.eau.EAU.lightBlock) {
                    count++
                    if (count == 2) break
                }
                vp.xCoord -= vv.xCoord
    ...
    fun setLightAt(coord: Coordonate, value: Int) {
        ...
        if (lbCoord == lamp.sixNode.coordonate) {
            ...
            org.ja13.eau.sixnode.lampsocket.LightBlockEntity.addLight(lbCoord, light, 5)
        }
        if (light != oldLight) {
